...
function ProductScreen() {
  ...
  const addToCartHandler = async () => {
    ...
    if (data.countInStock < quantity) {
      window.alert('lo siento, no nos quedan unidades');
      return;
    }
    ctxDispatch({
      type: 'CART_ADD_ITEM',
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    if (!comment || !rating) {
      toast.error('Por favor ingresa una reseña');
      return;
    }
    try {
                  ...
                  <Row>
                    ...
                    <Col>
                      {product.countInStock > 0 ? (
                        <Badge className="success">Disponible</Badge>
                      ) : (
                        <Badge className="danger">Agotado</Badge>
                      )}
                    </Col>
                  </Row>
                </ListGroup.Item>
